local string, find, sub, gsub, gmatch, len, tostring, tonumber, table, insert, sort, math, min, max, pairs, select, strsplit = string, find, sub, gsub, gmatch, len, tostring, tonumber, table, insert, sort, math, min, max, pairs, select, strsplit;
...
function cPowaBuffBase:AddEffectAndEvents()
	...
	if not self.target and not self.targetfriend and not self.party and not self.raid and not self.groupOrSelf and not self.focus and not self.optunitn then -- Self-buff
...
{
	buffNameTooltip = PowaAuras.Text.aideBuff,
	exactTooltip = PowaAuras.Text.aideExact,
	typeText = PowaAuras.Text.AuraType[PowaAuras.BuffTypes.Buff],
	mineText = PowaAuras.Text.nomMine,
	mineTooltip = PowaAuras.Text.aideMine,
...
cPowaDebuff.OptionText =
{
	buffNameTooltip = PowaAuras.Text.aideBuff2,
	exactTooltip = PowaAuras.Text.aideExact,
...
function cPowaCombo:CheckIfShouldShow(giveReason)
	if (PowaAuras.playerclass ~= "ROGUE" and PowaAuras.playerclass ~= "DRUID") then
		if (self.Debug) then
			PowaAuras:Message("cPowaCombo CheckIfShouldShow Class=", PowaAuras.playerclass);
		end
...
function cPowaAggro:AddEffectAndEvents()
	...
	if not self.target and not self.targetfriend and not self.party and not self.raid and not self.focus and not self.optunitn then -- Self aggro
...
function cPowaPvP:AddEffectAndEvents()
	...
	if (self.target or self.targetfriend) then -- Target pvp flag
...
cPowaSpellAlert.OptionText =
{
	...
	mineText = PowaAuras.Text.nomCanInterrupt,
	mineTooltip = PowaAuras.Text.aideCanInterrupt,
	extraText = PowaAuras.Text.nomOnMe,
	extraTooltip = PowaAuras.Text.aideOnMe,
	targetFriendText = PowaAuras.Text.nomCheckFriend,
	targetFriendTooltip = PowaAuras.Text.aideTargetFriend,
};
...
function cPowaSpellAlert:AddEffectAndEvents()
	...
	if self.groupOrSelf then -- Group or self casts
		player = false;
		table.insert(PowaAuras.AurasByType.GroupOrSelfSpells, self.id);
	end
	if player then -- Player casts
...
function cPowaSlots:CheckIfShouldShow(giveReason)
	if (self.Debug) then
		PowaAuras:Message("Slots Aura CheckIfShouldShow buffname=", self.buffname);
	end
	for pword in string.gmatch(self.buffname, "[^/]+") do
		pword = self:Trim(pword);
		if (string.len(pword) > 0) then
...
cPowaItems.OptionText =
{
	buffNameTooltip = PowaAuras.Text.aideItems, typeText = PowaAuras.Text.AuraType[PowaAuras.BuffTypes.Items],
...
function cPowaItems:CheckIfShouldShow(giveReason)
	if (self.Debug) then
		PowaAuras:Message("Items Aura CheckIfShouldShow buffname=", self.buffname);
	end
	for pword in string.gmatch(self.buffname, "[^/]+") do
		pword = self:Trim(pword);
		if (string.len(pword) > 0) then
